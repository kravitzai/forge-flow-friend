...
40. [`toolchains-phase-6i-e-cisco-to-brocade-validation-diff-engine.md`](./toolchains-phase-6i-e-cisco-to-brocade-validation-diff-engine.md)
    — Cisco → Brocade Validation/Diff Engine, advisory only (Phase 6I-E). Pure validator under `src/lib/canonicalZoning/validation/` that compares the Cisco source, current Brocade target, and 6I-D planned target. Emits `ValidationReport` with readiness, structured diffs, findings, blockers, and review items. Pinned exact-match golden report. No registry, runtime, edge function, CMS, or UI changes.
41. [`toolchains-phase-6i-f-pseudo-command-descriptors.md`](./toolchains-phase-6i-f-pseudo-command-descriptors.md)
    — Pseudo-Command Descriptors, advisory only (Phase 6I-F). Pure layer under `src/lib/canonicalZoning/pseudoCommands/` consuming a 6I-D `ConversionPlan` + 6I-E `ValidationReport`, emitting non-vendor descriptors (`alias-create`, `alias-rename`, `zone-create`, `zone-reconcile`, `cfg-extend`, `cfg-active-review`, `review-only`). Readiness gates generation; `blocked` returns `generated=false`; `needs-review` collapses review-required actions to `review-only`. Every template starts with `# advisory only — not executable`. Safety guard rejects forbidden keys/phrases. Pinned exact-match golden report. No registry, runtime, edge function, CMS, or UI changes.
26. [`toolchains-phase-6j-a-pseudo-command-review-ui.md`](./toolchains-phase-6j-a-pseudo-command-review-ui.md)
    — Pseudo-Command Review UI, read-only (Phase 6J-A). Page at `/toolchains/san-migration/review` rendering a 6I-F `PseudoCommandReport`. Operational-status grouping (Blocked / Requires Human Review / Advisory Ready / Suppressed). Report + Pipeline + Demo input modes, all browser-local. JSON + Markdown packet export. No execute/apply/run/push/activate controls; lint-enforced forbidden-label and forbidden-import scan. No registry, CMS, Supabase, edge, or runtime changes.
27. [`toolchains-phase-6j-b-cisco-to-brocade-zoning-conversion-advisor.md`](./toolchains-phase-6j-b-cisco-to-brocade-zoning-conversion-advisor.md)
    — Cisco → Brocade Zoning Conversion Advisor Engine (Phase 6J-B). First `migration-advisor` archetype. Wraps the pure 6I-D → 6I-E → 6I-F pipeline as a registered, browser-local engine. Dedicated `CiscoToBrocadeAdvisorPanel` (the generic `EngineActionPanel` is suppressed via `hiddenFromActionPanel: true`). Browser-local `sessionStorage` handoff to the 6J-A review page. Descriptor-only audit edge function with shared exact-key forbidden-key walker (rejects canonical models, MappingRules, template lines, WWPNs, aliases/zones/cfg names, raw evidence). Audit failure never blocks the inline result. No raw collection, no executable commands, no canonical-model storage, no runtime/vendor adapter calls.
