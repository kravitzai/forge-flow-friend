...
40. [`toolchains-phase-6i-e-cisco-to-brocade-validation-diff-engine.md`](./toolchains-phase-6i-e-cisco-to-brocade-validation-diff-engine.md)
    — Cisco → Brocade Validation/Diff Engine, advisory only (Phase 6I-E). Pure validator under `src/lib/canonicalZoning/validation/` that compares the Cisco source, current Brocade target, and 6I-D planned target. Emits `ValidationReport` with `readiness` (`blocked` / `needs-review` / `ready-for-pseudo-command-review` / `ready-for-lab-validation`), structured diffs (alias / zone / zoneset / mapping / safety), findings, blockers, and review items. Enforces no-command boundary by scanning planned actions for forbidden keys/phrases. Pinned exact-match golden report. No registry, runtime, edge function, CMS, or UI changes. Next: 6I-F (pseudo-command descriptors).
